    ...
    const fetchEvents = async () => {
      try {
        const response1 = await axios.get(
          "http://127.0.0.1:8000/event_fetch_data",
        );
        // console.log(response1.data)
        const response2 = await axios.get(
          "http://127.0.0.1:8000/event-booking-count",
        );
              ...
              <div className="group transition-transform hover:-translate-y-1 hover:shadow-xl bg-white/70 backdrop-blur-md rounded-2xl border border-gray-200 shadow p-5 cursor-pointer hover:bg-white duration-300">
                <div className="flex flex-col h-full justify-between">
                  ...
                  <div className="mt-4 text-sm text-gray-600">
                    <p>
                      ...
                      {e.location.charAt(0).toUpperCase() +
                        e.location.slice(1).toLowerCase()}
                    </p>
                  </div>
                  ...
                  <div className="mt-4 flex items-center justify-between gap-2">
                    ...
                    <span className="text-gray-500 text-sm">
                      Organizer: <span className="">{e.organizer.name}</span>
                    </span>
                  </div>
                </div>
              </div>
